...
# 新增：每輪每個特徵的驗證集 AUROC 紀錄
feature_auroc_results = []

# # 新增：使用 Val_AUROC > 0.8 特徵重算 sum 後的 CT 結果
# ct_selected_only_results = []

# # 新增：使用 Val_AUROC > 0.8 特徵重算 sum 後，再與 RF 融合
# ct_selected_rf_results = []

FEATURE_AUROC_THRESHOLD = 0.8

def compute_selected_ct_raw_by_val_auroc(
    round_id,
    ct_val_df,
    ct_test_df,
    y_val,
    ...
    ct_val_clean = ct_val_df.replace([np.inf, -np.inf], np.nan).fillna(0.0).copy()
    ct_test_clean = ct_test_df.replace([np.inf, -np.inf], np.nan).fillna(0.0).copy()

    feature_rows = []
    feature_cols = list(ct_val_clean.columns)

    for feature in feature_cols:
        val_score = -ct_val_clean[feature].values
        test_score = -ct_test_clean[feature].values

        try:
            val_auroc = roc_auc_score(y_val, val_score)
        except ValueError:
            val_auroc = np.nan

        try:
            test_auroc = roc_auc_score(y_test, test_score)
        except ValueError:
            test_auroc = np.nan
    ...
    feature_metrics_df = pd.DataFrame(feature_rows)

    selected_features = feature_metrics_df[
        feature_metrics_df["Val_AUROC"] >= auroc_threshold
    ]["feature"].tolist()

    # 避免沒有任何特徵通過門檻
    if len(selected_features) == 0:
        selected_features = [
            feature_metrics_df.sort_values("Val_AUROC", ascending=False).iloc[0]["feature"]
        ]

    # 在同一個 feature AUROC CSV 裡標記是否被選到
    for row in feature_rows:
        row["AUROC_threshold"] = auroc_threshold
        row["selected"] = int(row["feature"] in selected_features)

    feature_auroc_results.extend(feature_rows)

    # 每輪所有 feature 的 Val_AUROC 都累積到同一個 CSV
    pd.DataFrame(feature_auroc_results).to_csv(
        os.path.join(artifact_dir, "ct_feature_val_auroc_by_round.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    # 只用 selected features 重新計算 sum
    ct_val_raw = -ct_val_clean[selected_features].sum(axis=1).values
    ct_test_raw = -ct_test_clean[selected_features].sum(axis=1).values

    return ct_val_raw, ct_test_raw, selected_features
# -------------------- 主流程 --------------------
if __name__ == "__main__":
    ...
    t0 = now()
    ct_val_raw, ct_test_raw, ct_selected_features = compute_selected_ct_raw_by_val_auroc(
        round_id=0,   # 增量迴圈內改成 round_id=round_id
        ct_val_df=ct_val_df,
        ct_test_df=ct_test_df,
        y_val=y_val,
        y_test=y_test,
        artifact_dir=ARTIFACT_DIR,
        feature_auroc_results=feature_auroc_results,
        auroc_threshold=FEATURE_AUROC_THRESHOLD
    )
    ...
    while len(rem_0) > 0 and len(rem_1) > 0:
        ...
        t0 = now()
        ct_val_raw, ct_test_raw, ct_selected_features = compute_selected_ct_raw_by_val_auroc(
            round_id=round_id,   # 增量迴圈內改成 round_id=round_id
            ct_val_df=ct_val_df,
            ct_test_df=ct_test_df,
            y_val=y_val,
            y_test=y_test,
        ...
        def log_round_k():
            common = {
                ...
                'mlp_pred_test_time_s': mlp_pred_test_time_s,
                'ct_balance_enabled': CT_BALANCE_ENABLED,
                'feature_auroc_threshold': FEATURE_AUROC_THRESHOLD,
                'ct_selected_feature_count': len(ct_selected_features),
                'ct_selected_features': "|".join(ct_selected_features),
            }
