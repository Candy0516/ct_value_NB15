...
def map_testset(file_path, save_path, label_column, map_path, print=print):
    ...
    # 統計各column缺值總數
    total_massing = {}
    missing_mask_all = np.zeros(len(test), dtype=bool)
    # p值缺值補0.5，ct值缺值補0
    for c, col in enumerate(column_list):
        ...
        # ===== 找出 mapping 後缺值 =====
        nan_mask = pvalue_test[col].isna()
        # 記錄所有缺值 row
        missing_mask_all |= nan_mask
    ...
    #紀錄CT缺值資料-------------------------------------------
    rows_with_missing = benign_test[missing_mask_all]
    rows_without_missing = benign_test[~missing_mask_all]

    rows_without_missing.to_csv(
        os.path.join(save_path,'benign_test_without_missing.csv'),
        index=False
    )

    rows_with_missing.to_csv(
        os.path.join(save_path,'benign_test_with_missing.csv'),
        index=False
    )
...
def map_valset(file_path, save_path, label_column, map_path, print=print):
    val = pd.read_csv(file_path, low_memory=False)

    column_list = val.columns.to_list()
    #map_path = os.path.join('data', '2_score', 'statistic', 'map_table')
    ...
    for c, col in enumerate(column_list):
        ...
        pvalue_val[col] = pvalue_val[col].map(pvalueSeries).astype(float)
        # 統計缺值
        nanValuesList = val[col][pvalue_val[col].isna()].tolist()
        total_massing[col] = len(nanValuesList)
        listtocsv(nanValuesList, os.path.join(save_path, 'missing'), str(c)+'_'+col)
        # 缺值補0.5
        pvalue_val[col] = pvalue_val[col].fillna(0.5).astype(float)

        benign_val[col] = pvalue_val[col] - 0.5
        
    pvalue_val.to_csv(os.path.join(save_path, 'pvalue_val.csv'), index=None)
    benign_val.to_csv(os.path.join(save_path, 'benign_val.csv'), index=None)
