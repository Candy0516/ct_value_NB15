...
# include derived feature
if "rate" in train_df.columns and "rate" not in float_names:
    float_names.append("rate")


# keep only existing columns
nominal_names = [c for c in nominal_names if c in train_df.columns]
integer_names = [c for c in integer_names if c in train_df.columns]
float_names   = [c for c in float_names   if c in train_df.columns]
binary_names  = [c for c in binary_names  if c in train_df.columns]

# 切 validation（ 80% train / 20% val）
train_df_split, val_df = train_test_split(
    train_df,
    test_size=0.2,
    random_state=42,
    stratify=train_df['label']  # 強烈建議（避免類別失衡）
)
...
train_proc.to_csv(
    "UNSW_NB15_train_normalized.csv",
    index=False
)
